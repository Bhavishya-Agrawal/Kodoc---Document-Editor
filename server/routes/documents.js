...
// Update document title and optional metadata (wordCount)
router.put("/:id", async (req, res) => {
  try {
    ...
    if (wordCount !== undefined && typeof wordCount === "number") {
...
router.patch("/:id/star", async (req, res) => {
  try {
    ...
    res.json({ document, isStarred: document.isStarred });
  } catch (err) {
    console.error(err.message);
    if (err.kind === "ObjectId") {
      return res.status(404).json({ message: "Document not found" });
...
// Duplicate an existing document along with its latest content
router.post("/:id/duplicate", async (req, res) => {
  try {
    const sourceDoc = await Document.findById(req.params.id);

    if (!sourceDoc) {
      return res.status(404).json({ message: "Document not found" });
    }

    if (sourceDoc.owner.toString() !== req.user.id) {
      return res.status(401).json({ message: "Not authorized" });
    }

    // Get latest version content
    let contentToCopy = "<p></p>";
    if (sourceDoc.latestVersionId) {
      const v = await Version.findById(sourceDoc.latestVersionId);
      if (v) contentToCopy = v.content;
    } else {
      const v = await Version.findOne({ documentId: sourceDoc._id }).sort({ versionNumber: -1 });
      if (v) contentToCopy = v.content;
    }

    const newTitle = await generateUniqueTitle(req.user.id, `${sourceDoc.title} (Copy)`);
    ...
    const initialVersion = new Version({
      ...
      commitMessage: `Duplicated from "${sourceDoc.title}"`,
      action: "initial",
      contentSizeBytes: Buffer.byteLength(contentToCopy, "utf8"),
      createdBy: req.user.id,
      createdAt,
      updatedAt: createdAt,
    });
    ...
    newDoc.latestVersionId = initialVersion._id;
    await newDoc.save();

    res.status(201).json({ document: newDoc, initialVersion });
  } catch (err) {
    console.error(err.message);
    if (err.kind === "ObjectId") {
      return res.status(404).json({ message: "Document not found" });
