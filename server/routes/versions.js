...
router.post("/:docId/versions", verifyDocumentOwnership, async (req, res) => {
  try {
    ...
    const newVersion = new Version({
      ...
      wordCount: countWordsFromHtml(normalizedContent),
    ...
    // Update the document's current version and word count
    document.currentVersion = newVersionNumber;
    document.latestVersionId = newVersion._id;
    document.wordCount = countWordsFromHtml(normalizedContent);
    await document.save();
...
router.post("/:docId/versions/:versionId/restore", verifyDocumentOwnership, async (req, res) => {
  try {
    ...
    const restoredVersion = new Version({
      ...
      wordCount: countWordsFromHtml(oldVersion.content || ""),
      createdBy: req.user.id,
      createdAt,
      updatedAt: createdAt,
    });
    ...
    document.wordCount = countWordsFromHtml(oldVersion.content || "");
    await document.save();
